      ...
      <div class="card-body">
          <div class="row" style="margin-top:-20px;">
            <div class="col-md-6 menu">
              <nav class="navecation" style="margin-left:-40px;margin-top:20px;">
                <ul id="navi">
                  <li><a class="menu {{(request()->is('applicants*')) ? 'current' : '' }}" href="{{route('applicants.index')}}">All ({{$count}})</a></li>
          ...
          <hr>
            ...
            <div style="float:right;">
              <form action="{{route('applicants.index')}}">
        ...
        @foreach($data as $key => $user)
          <tr>
            ...
            <td>
              <a href="{{ route('applicants.show', $user->id) }}" class="btn btn-primary btn-sm">Detail</a>
              <form class="d-inline" id="#submitTrash" action="{{route('applicants.destroy', ['id' => $user->id])}}" method="POST">
                @csrf 
...
<script>
  ...
  document.getElementById('pagination').onchange = function() { 
    ...
    window.location = "{{URL::route('applicants.index')}}?items=" + this.value; 
  }; 
                        ...
                        if (data['success']) {
                          alert(data['success']);
                            $(".sub_chk:checked").each(function() {  
                              ...
                              window.location = "{{URL::route('applicants.index')}}"; 
                            });
