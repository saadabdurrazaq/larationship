...
@section("content")
  ...
  <div class="card card-secondary">
      <div class="card-header">
          <h3 class="card-title">Trash Applicants</h3>
      </div>
      <div class="card-body">
            ...
            <div class="col-md-6">
              <nav class="navecation" style="margin-left:-40px;margin-top:20px;">
                <ul id="navi">
                  ...
                  <li><a class="menu {{(request()->is('applicants*')) ? 'current' : '' }}" href="{{route('applicants.trash')}}">Trash ({{$countTrash}})</a></li>
                </ul>
              </nav>
            </div>
          </div>
          <hr>
            ...
            <div style="float:right;">
              <form action="{{route('applicants.trash')}}">
        ...
        @foreach($data as $key => $user)
          <tr>
            ...
            <td>
              <form class="d-inline" id="#submitRestore" action="{{route('applicants.restore', ['id' => $user->id])}}" method="POST">
                @csrf 
              ...
              <form class="d-inline" id="#submitDelete" action="{{route('applicants.delete-permanent', ['id' => $user->id])}}" method="POST">
                @csrf 
...
<script>
  ...
  document.getElementById('pagination').onchange = function() { 
    ...
    window.location = "{{URL::route('applicants.trash')}}?items=" + this.value; 
  }; 
                      ...
                      if (data['success']) {
                        alert(data['success']);
                          $(".sub_chk:checked").each(function() {  
                            ...
                            window.location = "{{URL::route('applicants.trash')}}"; 
                          });
