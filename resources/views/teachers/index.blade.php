...
<div id="flash-message">
  ...
  <div class="card card-secondary"> 
      <div class="card-header">
          <h3 class="card-title">Students & Teachers</h3>
      </div>
      <div class="card-body">
          <div class="row" style="margin-top:-20px;">
            ...
            <div class="col-md-12 text-right">
              <a href="{{ route('teachers.create') }}" class="btn btn-primary">Add New Teachers</a>
              <a href="{{ route('students.index') }}" class="btn btn-primary">List of Students</a>
            </div>
          </div>
          <hr>
        ...
        @foreach($data as $teacher)
          <tr>
            ...
            <td>
              <?php $elements = array(); ?>
              @foreach($teacher->student as $category)
                <?php $elements[] = $category->name; ?>
              @endforeach
              <?php echo implode(', ', $elements); ?>  
            </td>
            <td> 
              <a href="{{ route('teachers.assign-student', $teacher->id) }}" style="margin-top:5px;" class="btn btn-success btn-sm">Assign</a> 
              <a href="{{ route('teachers.edit-type', $teacher->id) }}" style="margin-top:5px;" class="btn btn-primary btn-sm">Edit</a> 
              <form class="d-inline" id="#submitDelete" action="{{ route('teacher.delete-permanent',['id'=>$teacher->id]) }}" method="POST">
                @csrf 
...
<script>
  ...
  document.getElementById('pagination').onchange = function() { 
    ...
    window.location = "{{URL::route('teachers.index')}}?items=" + this.value; 
  }; 
