      ...
      <div class="card-body">
        ...
        @foreach($data as $job)
          <tr>
            <td>{{ $job->name }}</td> 
            <td> 
              <a href="{{ route('jobs.edit-job', $job->id) }}" style="margin-top:5px;" class="btn btn-primary btn-sm">Edit</a> 
              <form class="d-inline" id="#submitDelete" action="{{ route('job.delete-permanent', ['id' => $job->id]) }}" method="POST">
                @csrf 
