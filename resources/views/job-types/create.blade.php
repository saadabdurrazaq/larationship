...
<div class="card card-secondary">
    <div class="card-header">
        <h3 class="card-title">Create Type of Jobs</h3>
    </div>
    <div class="card-body">
        <!-- enctype="multipart/form-data" karena kita akan mengunggah (mengupload) file dari form -->
        <form action="{{ route('job-types.store') }}" method="POST" id="formKirim" files="true" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-6">
                    <label for="name">Type of Jobs</label> 
                    <input type="text" placeholder="Type of Jobs" class="form-control {{ $errors->first('type') ? "is-invalid": "" }}" name="type" id="type" value="{{ old('type') }}" />
