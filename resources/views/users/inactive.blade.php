...
@section("title") Inactive Users list @endsection 

@section('index-admin-list')
{{ Breadcrumbs::render('list-inactive-admin') }}
@endsection
...
@section("content")
  ...
  <div class="card card-secondary">
      <div class="card-header">
          <h3 class="card-title">List Inctive User</h3>
      </div>
      <div class="card-body">
            ...
            <div class="col-md-6">
              <nav class="navecation" style="margin-left:-40px;">
                <ul id="navi" style="margin-top:40px;">
                  ...
                  <li><a class="menu {{(request()->is('users*')) ? 'current' : '' }}" href="{{route('users.inactive')}}">Inactive ({{$inactiveStatus}})</a></li>          
                </ul>
              </nav>
            </div>
          ...
          <hr>
            ...
            <div style="float:left;margin-left:4px;">
              <select class="form-control select2bs4 select2-hidden-accessible deactivate_all" style="width:130px;"" data-select2-id="17" tabindex="-1" aria-hidden="true">
                ...
                <option data-select2-id="38" value="deactivateAll">Activate</option>
              </select>
            </div>
            @endcan
            <div style="float:left;margin-left:20px;">
              <select onchange="window.location = this.options[this.selectedIndex].value" class="form-control select2bs4 select2-hidden-accessible download-doc" style="width:115px;"" data-select2-id="17" tabindex="-1" aria-hidden="true">
                <option selected="selected" data-select2-id="19" value="{{route('users.inactive')}}">Download</option>
                <option data-select2-id="38" value="{{route('users.pdfinactiveadmin')}}">PDF</option>
                <option data-select2-id="39" value="{{route('users.inactiveexcel')}}">XLS</option>
                <option data-select2-id="39" value="{{route('users.wordinactiveadmin')}}">Doc</option>
              </select>
            </div>
        ...
        @foreach($data as $key => $user)
          <tr>
            ...
            <td>
              @can('Activate Users')
              <form class="d-inline" action="{{route('users.activate', ['id' => $user->id])}}" method="POST" onsubmit="return confirm('Activate user?')">
                @csrf 
              <input type="hidden" value="get" name="_method">
                <input type="submit" style="margin-top:5px;" class="btn btn-success btn-sm" value="Activate">
              </form>
              @endcan    
            </td>
          </tr>
...
<script>
  ...
  document.getElementById('pagination').onchange = function() { 
    ...
    window.location = "{{URL::route('users.inactive')}}?items=" + this.value; 
  }; 
      ...
      if($(this).val() == "deactivateAll") {
        ...
        if(allVals.length <= 0)  {  
          alert("Please select row."); 
            ...
            var check = confirm("Are you sure you want to activate this row?");  
            if(check == true){  

                var join_selected_values = allVals.join(","); 

                $.ajax({
                    url: '{{ url('usersadminActivateAll') }}',
                    type: 'get',
                        ...
                        if (data['success']) {
                          alert(data['success']);
                            $(".sub_chk:checked").each(function() {  
                              ...
                              window.location = "{{URL::route('users.inactive')}}"; 
                            });
  ...
  $('.download-doc').on('change', function(e) { 
    if ($(this).val() == "{{route('users.pdfinactiveadmin')}}") {
      var url = this.value;  
      window.open(url, '_blank');
      $("#whole_page_loader").show();
    } 
    if($(this).val() == "{{route('users.inactiveexcel')}}") {
      var url = this.value;  
      window.open(url, '_blank');
      $("#whole_page_loader").show();
    ...
    if($(this).val() == "{{route('users.wordinactiveadmin')}}") {
      var url = this.value;  
      window.open(url, '_blank');
      $("#whole_page_loader").show();
      $.get(url, function(file_url) {
