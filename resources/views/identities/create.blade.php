...
@section("title") Create New Identities @endsection
...
<div class="card card-secondary">
    <div class="card-header">
        <h3 class="card-title">Create New Identities</h3>
    </div>
    <div class="card-body">
        <!-- enctype="multipart/form-data" karena kita akan mengunggah (mengupload) file dari form -->
        <form action="{{ route('identities.store') }}" method="POST" id="formKirim" files="true" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-6">
                    <label for="name">Identities</label> 
                    <input type="text" placeholder="Identities" class="form-control {{ $errors->first('identities') ? "is-invalid": "" }}" name="identities" id="identities" value="{{ old('identities') }}" />
