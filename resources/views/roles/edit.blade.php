...
@section('index-admin-list')
    {{ Breadcrumbs::render('list-role-edit') }}
@endsection
            ...
            <div class="col-xs-6 col-sm-6 col-md-6 panel-body table-responsive" style="overflow:hidden;">
                <table class="table table-hover table-striped dataTable" style="border:1px solid rgba(0,0,0,.125);">
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <input type="checkbox" name="permission[]" data-bootstrap-switch data-off-color="danger" value="{{ $value->id }}" {{ in_array($value->id, $rolePermissions) ? 'checked':'' }}> {{ $value->name }} <br>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
