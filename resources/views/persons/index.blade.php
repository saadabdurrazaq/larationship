...
<div id="flash-message">
  ...
  <div class="card card-secondary"> 
      <div class="card-header">
          <h3 class="card-title">Persons & Their Identities</h3>
      </div>
      <div class="card-body">
          <div class="row" style="margin-top:-20px;">
            ...
            <div class="col-md-12 text-right">
              <a href="{{ route('persons.create') }}" class="btn btn-primary">Add New Persons</a>
              <a href="{{ route('identities.index') }}" class="btn btn-primary">List of Identities</a>
            </div>
          </div>
          <hr>
        ...
        @foreach($data as $person)
          <tr>
            <td>{{ $person->name }}</td>  
            <td>
              {{ optional($person->identity)->no_ktp }}
            </td>
            <td> 
              <a href="{{ route('persons.assign-identity', $person->id) }}" style="margin-top:5px;" class="btn btn-success btn-sm">Assign</a> 
              <a href="{{ route('persons.edit-person', $person->id) }}" style="margin-top:5px;" class="btn btn-primary btn-sm">Edit</a> 
              <form class="d-inline" id="#submitDelete" action="{{ route('person.delete-permanent',['id'=>$person->id]) }}" method="POST">
                @csrf 
...
<script>
  ...
  document.getElementById('pagination').onchange = function() { 
    ...
    window.location = "{{URL::route('persons.index')}}?items=" + this.value; 
  }; 
